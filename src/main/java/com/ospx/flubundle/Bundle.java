...
public class Bundle {
    ...
    public void addSource(Fi directory) {
        directory.walk(fi -> {
            ...
            var codes = localeCode.toLowerCase().split("_");
            var locale = codes.length >= 2
                    ? Locale.of(codes[0], codes[1].toUpperCase())
                    : Locale.of(codes[0]);
    ...
    public Locale locale(String code) {
        ...
        var locale = codes.length >= 2 ? Locale.of(codes[0], codes[1]) : Locale.of(codes[0]);
